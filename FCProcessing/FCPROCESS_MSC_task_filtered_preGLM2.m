...
for i=1:numsubs
    ...
    for j=1:size(QC(i).restruns,1)
        ...
        % set original data - CG: edit to point to residuals files - but not here because these are based on pre-regression
        ...
        boldimg{i,j} = cell2mat(strcat(df.filepath{i,1},'/orig_files/',QC(i).vcnum,'_b',task,QC(i).restruns(j,:),'_faln_dbnd_xr3d_uwrp_atl.4dfp.img'));
        boldifh{i,j} = cell2mat(strcat(df.filepath{i,1},'/orig_files/',QC(i).vcnum,'_b',task,QC(i).restruns(j,:),'_faln_dbnd_xr3d_uwrp_atl.4dfp.ifh'));
